...
const PolarGraph = ({ candidates }) => {
    ...
    useEffect(() => {
        const candidatesTerrirtory = candidates?.candidates?.reduce((acc, candidate) => {
            acc[candidate.city] = (acc[candidate.city] || 0) + 1;
            return acc;
        }, {});
        if (Object.keys(candidates)?.length > 0) setCandidatesCount(candidatesTerrirtory);
    }, [candidates]);

    const data = {
        labels: candidatesCount && Object.keys(candidatesCount)?.length > 0 ? Object.keys(candidatesCount) : [],
        datasets: [
            {
                label: "Popularity",
                data: candidatesCount && Object.keys(candidatesCount)?.length > 0 ? Object.values(candidatesCount) : [],
                backgroundColor: ["#2CD9C5", "#826AF9", "#FFE700", "#FF6C40", "#2D99FF", "#505D6F"],
                borderWidth: 1,
            },
        ],
    };
