                ...
                <nav>
                    <ul className='flex items-center gap-2'>
                        {navItems.map((item, index) => (
                            <li
                              key={index}
                              className={`p-3  ${location.pathname === item.path ? 'font-bold' : 'font-medium'}  cursor-pointer hover:text-[#0A6562] ${location.pathname === item.path ? 'text-[#0A6562]' : 'text-[#5D5D5D]'} ${location.pathname === item.path ? 'border-t-3 border-[#0A6562]' : 'border-transparent'}`}
                            >
