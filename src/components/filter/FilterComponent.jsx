    ...
    case "SET_KPI_SELECTED_OPTIONS":
      return {
        ...
        selected_kpis: {
          ...
          [action.payload.kpi]: {
            ...state.selected_kpis[action.payload.kpi],
            selected_options: state.selected_kpis[
              action.payload.kpi
            ]?.selected_options?.includes(action.payload.selected_options)
              ? state.selected_kpis[
                  action.payload.kpi
              ...
              : [
                  ...(Array.isArray(
                    state.selected_kpis[action.payload.kpi]?.selected_options
                  )
                    ? state.selected_kpis[action.payload.kpi].selected_options
                    : []),
...
const FilterComponent = ({ fetchCandidates }) => {
  ...
  return (
    <div
      className="shadow-[0px_3px_15px_#75757529] w-[300px] max-h-[800px] overflow-auto"
      ...
      <div className="text-sm flex flex-col gap-4 py-2 px-4">
              ...
              <div className="absolute z-10 mt-1 w-full bg-[#f6f6f6] border border-gray-300 rounded shadow p-2 max-h-60 overflow-y-auto">
                {subDomains.map((subDomain) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={state?.subdomain
                        ?.split(",")
                        ?.includes(subDomain.value)}
                      onChange={() =>
                        dispatch({
                          type: "TOGGLE_SUBDOMAIN",
                          payload: subDomain.value,
                        })
                      }
        ...
        <div className="flex flex-col">
          ...
          <div className="relative">
            <select
              ...
              value={state?.job}
              onChange={(e) =>
                dispatch({ type: "TOGGLE_JOB_FIELD", payload: e.target.value })
              }
              className="appearance-none w-full bg-white border border-[#C9C9C9] p-2 px-4 rounded-full leading-tight focus:outline-none focus:ring-1 focus:ring-[#C9C9C9] overflow-hidden"
            >
        ...
        <div className="flex flex-col">
          ...
          <div className="relative">
            <select
              ...
              value={state?.top_k}
              onChange={(e) =>
                dispatch({ type: "SET_TOPK", payload: e.target.value })
              }
              className="appearance-none w-full bg-white border border-[#C9C9C9] p-2 px-4 rounded-full leading-tight focus:outline-none focus:ring-1 focus:ring-[#C9C9C9] overflow-hidden"
            >
        ...
        <div className="flex flex-col">
          ...
          <input
            ...
            value={state?.exp}
            onChange={(e) =>
              dispatch({ type: "SET_EXPERIENCE", payload: e.target.value })
            }
            className="w-full h-2 bg-[#C9C9C9] rounded-lg appearance-none cursor-pointer accent-white"
            style={{
              background: `linear-gradient(to right, #0A6562 ${
                ((state?.exp - 5) * 100) / 50
              }%, #ccc ${((state?.exp - 5) * 100) / 50}%)`,
            }}
          />
        </div>
        {/* Minimum fit % slider */}
        <div className="flex flex-col">
          ...
          <input
            ...
            onChange={(e) =>
              dispatch({ type: "SET_MINIMUM_FIT", payload: e.target.value })
            }
            className="w-full h-2 bg-[#C9C9C9] rounded-lg appearance-none cursor-pointer accent-white"
            style={{
              background: `linear-gradient(to right, #0A6562 ${
                ((state?.min_fit_score - 10) * 100) / 85
              }%, #ccc ${((state?.min_fit_score - 10) * 100) / 85}%)`,
            }}
          />
        </div>
      </div>
              ...
              <div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    checked={
                      state.selected_kpis[filterOption.value]?.enabled || false
                    }
                    onChange={(e) =>
                      dispatch({
                        ...
                        >
                          <input
                            ...
                            checked={
                              state.selected_kpis[
                                filterOption.value
                              ]?.selected_options?.includes(sub.value) || false
                            }
                            onChange={() =>
                              dispatch({
                    ...
                    if (state.selected_kpis[filterOption.value]?.weight < 100) {
                      dispatch({
                        ...
                        payload: {
                          ...
                          weight:
                            parseInt(
                              state.selected_kpis[filterOption.value]?.weight
                            ) + 1,
                        },
                      });
                    }
