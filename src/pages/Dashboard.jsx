...
const Dashboard = () => {
    ...
    const fetchCandidates = async (filter) => {
        ...
        try {
            ...
            if (totalWeight < 100) {
                alert(`Total weight is less than 100 (${totalWeight}). Please adjust weights.`);
                return;
            }
            if (totalWeight > 100) {
                alert(`Total weight exceeds 100 (${totalWeight}). Please adjust weights.`);
                return;
            }
            const json = {
                field: [filter.Domain, ...filter.subdomain.split(",").map((s) => s.trim())],
            ...
            const response = await fetch("https://talent-poc.ctruh.com/profile-candidates", {
                method: "POST",
                headers: {
        ...
        <div className="w-full mt-3 flex">
            ...
            {Object.keys(candidates)?.length === 0 ? (
                <div className="flex flex-col items-center justify-center w-full h-full p-10 text-center text-gray-500">
                    ...
                    <p className="text-lg font-medium">Try adjusting your filter or KPIs criteria to show the data.</p>
                </div>
            ) : (
                <div className="w-full mx-4 flex flex-col gap-6">
                    ...
                    <div className="grid w-full sm:grid-cols-1 lg:grid-cols-3">
                        ...
                        <div className=" px-3 h-[300px] border-r-[#CAD5D5] border-r">
                            <h5 className="text-center font-bold">Average Candidate vs. AI Benchmark</h5>
                            <RadarGraph selectedFiltrs={selectedFiltrs} candidates={candidates} />
                        </div>
